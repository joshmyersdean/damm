...
def get_args() -> config.CfgNode:
	...
	parser.add_argument('--config', type=str, default='config/example.yaml', help='config file')
		...
		with open(args.proteome) as handle:
			for record in SeqIO.parse(handle, "fasta"):
				...
				if not constraints_met:
					continue
				free_positions = [i for i in free_positions if i not in constrained_pos]
				for pos in free_positions:
					if record.seq[pos] != src[pos]:
						...
						if subs > max_subs:
							constraints_met = False
				if constraints_met:
					f.write(f"{subs} \t {record.id} \t {record.seq[-6:]} \t {src}\n")
	if verbose:
